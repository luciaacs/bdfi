...
RESPONSE_TOPIC = 'flight_prediction_response' # nuevo topic de kafka para datos de salida

import uuid

from kafka import KafkaConsumer

consumer = KafkaConsumer( # este consumira los datos del topic de kafka
  'flight_delay_classification_response_kafka',
...
def on_time_performance():
  ...
  # aqui se guardan detalles especificos de un vuelo como el numero de vuelo, la aerolinea, la fecha...
...
def list_flights(origin, dest, flight_date):
  ...
  # aqui se guardan detalles especificos de un vuelo con una fecha especifica
...
def total_flights():
  ...
  # aqui se guardan estadísticas de vuelos totales organizadas por año y mes
...
def total_flights_chart():
  ...
  # aqui se guarda la misma info de total_flights pero con graficos o visualizaciones
...
# aqui se guardan detalles sobre los aviones como el fabricante, el modelo o el propietario
...
def flights_per_airplane(tail_number):
  ...
  # aqui se guardan los vuelos asociados a un avion
...
def airline(carrier_code):
  ...
  # aqui se guarda informacion de la aerolinea y sus aviones
...
def airlines():
  ...
  # aqui se listan todas las aerolineas
...
def search_flights():
  ...
  return render_template(
    ...
    # muestra una lista de vuelos filtrada según los parámetros de búsqueda del usuario
...
# Realiza la prediccion de retrasos y envia el resultado al topic de kafka
...
def flight_delays_page():
  ...
  # Formulario para ingresar datos de un vuelo y obtener la predicción.
...
def flight_delays_batch_page():
  ...
  # formulario para procesar multiples vuelos y realizar predicciones por lotes
...
def flight_delays_batch_results_page(iso_date):
  ...
  # muestra una lista de los resultados de las predicciones
...
def flight_delays_page_kafka():
  ...
  # muestra un formulario para que el usuario ingrese datos del vuelo y solicite una predicción
  # ESTE ES EL HTML CON LA INFO DEL DELAY QUE NECESITAMOS

  # # Enviar el resultado al tópico de Kafka (response) ----------nuevo
  # kafka_message = json.dumps(result_obj).encode("utf-8")
  # print("Enviando mensaje a Kafka:", kafka_message)
  # producer.send(RESPONSE_TOPIC, kafka_message)
  # producer.flush()
  
  # # Confirmar en la aplicación
  # response = {"status": "OK", "prediction": result_obj}
...
def classify_flight_delays_realtime_response(unique_id):
  ...
  #   # Generate the display message for the prediction
  #   prediction_text = get_display_message(prediction["Prediction"])

  #   # Send the prediction text to Kafka
  ...
  #return json_util.dumps(response)
  #calcula la prediccion, que es un numero
...
def get_display_message(prediction):
    ...
    if prediction == 0 or str(prediction) == '0':
        ...
        return "Slightly Early (0-15 Minutes Early)"
